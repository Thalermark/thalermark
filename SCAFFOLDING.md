...
| **10** | Production hardening + open-core seams | Make the product deploy-ready and add extension points (community defaults keep self-host whole) | 🚧 **Partial** — all six open-core seams shipped (#351–#356, #393–#395); hardening incomplete. Open: staff role still in the core baseline, email domain auth, Caddy edge rate-limit, secret rotation. Per-item status in the Phase 10 section |

Foundation shipped (Phases 0–7); Phase 8 delivered the full MVP web scope; Phase 9 brought the mobile app to feature parity. **Phase 10 is partially done** — the open-core seams are all in, the production hardening is not; the Phase 10 section carries a per-item status verified against the tree rather than inferred from PR titles. Substantial product work has also shipped *alongside* it, catalogued in the *Post-MVP polish* sections below (see PROJECT.md for scope).
...
| Error tracking reports in prod | ⚠️ **code done, deploy unconfirmed.** `api onError → captureException` + `@sentry/sveltekit` (#283/#284) and `ERROR_TRACKING_DSN` is documented — but nobody has confirmed events arriving in a live backend |
| **Staff read-only role in its own optional migration** | ❌ **NOT DONE — and it contradicts the plan of record.** `thalermark_staff_readonly` is still `CREATE ROLE`d in core at `0000_baseline.sql:29`, with grants in 0001/0002. `spikes/SAAS-AND-PRODUCTION.md` says this role belongs to the commercial migration pack so a self-host DB never has it. Today every self-host install gets a BYPASSRLS role it has no use for |
| **Email domain auth (SPF/DKIM)** | ❌ not addressed. Still on the beta punch list |
| **Caddy edge rate-limiting** | ❌ open — app-level limiting only, so an attacker still reaches Node |
| **Secret rotation** | ❌ open — no documented procedure for rotating `BETTER_AUTH_SECRET` / storage / DB credentials |

**Phase 10 is therefore NOT complete, and the remaining work is all hardening.** The two that
would embarrass a launch are the staff role (a self-host install shipping a BYPASSRLS role
nobody asked for) and email domain auth (invoice mail landing in spam). Neither is large; both
have been passed over because feature work kept arriving.
