...
- **Mobile develops in Expo Go and ships as a native build.** `expo start` serves JS over Metro to the installed Expo Go app, which is the fast loop. The native path also works: `expo prebuild --platform android` + `./gradlew assembleRelease` produces an installable APK (verified on a Pixel 8 Pro, PR #510). **A native build requires a JDK 17** and will not work with Android Studio's bundled JBR 21 alone. This note previously said not to run `expo run:android` because it crashed on a removed `JvmVendorSpec`; that crash was real but the diagnosis was not, and it stopped anyone from building the app for months. `@react-native/gradle-plugin` is an included build declaring `kotlin { jvmToolchain(17) }`, Gradle toolchains match on **exact** major version, so with no JDK 17 installed Gradle falls back to auto-provisioning, which runs Foojay 0.5.0, which references a field Gradle 9.3.1 deleted. Install a JDK 17 and Foojay is never invoked. Point Gradle at it with `org.gradle.java.installations.paths` in `~/.gradle/gradle.properties` (Homebrew's `openjdk@17` needs the `libexec/openjdk.jdk/Contents/Home` path; the symlink root is not a valid JDK home). The ref-gated once-only bootstrap pattern (M3) applies to any multi-fetch bootstrap.
