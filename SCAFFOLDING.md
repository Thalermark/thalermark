...
**Status:** Phases 0–7 shipped (2026-05-23); Phase 8 (MVP features) in progress — slices 8.1–8.4f, 8.5a–8.5c, 8.6a–8.6c merged (latest 2026-05-25). Invoice CRUD + status flow, the send-invoice chain (public view → email → Stripe self-host pay), and the customer-creation chain (inline create → dupe detection → address autocomplete) all complete. SaaS multi-tenant payment routing via Stripe Connect deferred to 8.5d / 8.5e.
...
| **8** | MVP features — customers + invoices first, then estimates / expenses / dashboard / AI | This is where the product becomes visible | 🚧 In progress (slices 8.1–8.4f, 8.5a–8.5c, 8.6a–8.6c, PRs #82–#84, #86, #92, #95, #97–#100, #102–#104, #107, #108, #110 — plus mid-phase footguns #87, #88, #90, #91 and quick-follow fix #96) |
