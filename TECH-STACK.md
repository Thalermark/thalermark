...
| **Location / address autocomplete** | **US Census geocoder (keyless default)**, Mapbox (token-gated upgrade), OpenStreetMap Nominatim (also available), behind portable `AddressAutocompleteProvider` interface | Census TIGER is free + keyless with strong US residential coverage — no signup for self-hosters; `MAPBOX_ACCESS_TOKEN` upgrades to Mapbox; same swap-via-config pattern as the LLM provider |
