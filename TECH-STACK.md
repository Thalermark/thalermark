...
| **Auth** | Better Auth | Cookies (web) + bearer tokens (mobile) in one library, official Drizzle + Hono adapters, self-hostable. Organizations plugin is **off** — tenancy (`account_id`/`company_id` + memberships) is our own domain, not Better Auth's. |
...
| **LLM abstraction** | Vercel AI SDK | Thin provider-agnostic seam — Anthropic / OpenAI / Ollama / custom-endpoint swap via `packages/ai` presets, uniform streaming + tool use + structured outputs |
| **BYOK** | Self-host: yes (in-app, per account — **Settings → AI**, encrypted at rest; the `LLM_*` env was removed); SaaS Pro: post-MVP | Self-host story requires it; privacy-sensitive users (accountants, NDA-bound) get real path via local Ollama; SaaS keeps curated experience with our key + quotas |
...
| **PDF handling** | None for invoices; `pdf-to-png-converter` for receipt OCR | Invoices are a public web view + email + Stripe pay link — no PDF artifact. Uploaded receipt PDFs render page-1 → PNG (pdfjs-dist + native canvas, no browser binary) for the vision LLM. Playwright (HTML→PDF + shared-Chromium E2E) was the original pick but was never adopted; the runtime stays lean (no ~300MB Chromium). |
| **Receipt OCR** | Two-layer: capture (core, all tiers) + extraction (Pro+/BYOK, vision LLM via Vercel AI SDK) | Image is always saved (IRS-compliant in every tier). AI auto-fills merchant/total/date/category as a premium feature. No separate vendor for MVP; the account's one configured AI connection (Settings → AI) powers OCR and insights alike. Vendor (Veryfi/Mindee) is a future Pro upsell as fallback for low-confidence extractions. |
...
| **Location / address autocomplete** | **Google Places API (New)**, behind a two-phase `AddressAutocompleteProvider` interface (autocomplete predictions → Place Details on pick, session-token billed). Server-wrapped so the key never reaches the browser and the CSP needs no Google origins | Reliable, global coverage; optional per deployment (`GOOGLE_PLACES_API_KEY` — unset ⇒ the field degrades to manual entry). Replaced the flaky Census/Mapbox/Nominatim trio (TMC-141) |
