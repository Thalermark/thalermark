...
// side. Two-step create when adding a contact inline; money math done with
...
export default function NewInvoice() {
  ...
  async function onSubmit() {
    ...
    if (inlineMode) {
      ...
      if (findEmailDupe(parsedCust.data.email, contacts)) return; // hard block (submit already disabled)
          ...
          <View className="mt-8 space-y-5">
            ...
            <View>
              ...
              {fieldErrors.contactId ? (
                <Text className="mt-1 text-xs text-oxblood">{fieldErrors.contactId}</Text>
              ) : null}
            </View>
