<script lang="ts">
  ...
  import type { OpeningBalanceData, OpeningBalanceForm } from '$lib/opening-balance';
  import { type UnmatchedRow, parseTrialBalance } from '$lib/trial-balance-import';
  import Papa from 'papaparse';
  import { untrack } from 'svelte';

  // What a business already had when it arrived — either three plain questions
  // (someone just getting going) or a full trial balance (someone leaving
  // QuickBooks or Xero mid-year).
  //
  // A component rather than a page because THREE hosts need it: the owner-money
  // route, Settings → Import & export, and the welcome wizard. There is exactly
  // one opening-balance record per company and whichever shape saves last wins,
  // so two implementations would eventually let one wipe the other. Server side
  // is shared too — see $lib/opening-balance.server.ts.
  //
  // The host owns its own heading; this owns the forms.
  let {
    data,
    form,
    cancelHref,
  ...
  // The full trial balance is only offered to roles that can post adjustments —
  // the loader returns an empty chart for everyone else. The three plain
  // questions stay the default: they're the right first ask, and most people
  // starting out have nothing else to say.
  const canEnterFull = $derived(data.accounts.length > 0);

  type Line = { coaAccountId: string; side: 'debit' | 'credit'; amount: string };

  // Seeded once, then owned by the form. `untrack` says that deliberately:
  // these are the starting values of an editable draft, not a view of `data` —
  // without it Svelte warns that only the initial value is captured, which is
  // exactly what's wanted (same pattern as the ledger list's row state).
  ...
  // Fills the editor below rather than saving. The user sees what matched,
  // fixes what didn't, and the balance check still has to pass — an import that
  // wrote straight to the ledger would be a way to get someone else's rounding
  // errors into your books without looking at them.
...
{#if !advanced}
  <form method="post" action="?/save" class="mt-8 space-y-6" use:enhance>
    <div>
      <label for="asOfDate" class="label">
        When did you start?<span class="text-accent">*</span>
      </label>
  ...
  <div class="mt-8 border-t border-fg/10 pt-6">
    ...
    {#if !advanced}
      <p class="mt-2 max-w-2xl text-sm text-fg/50">
        ...
        you've already written down, a loan, tax you've collected. Enter your full opening balances
        instead.
      </p>
    {/if}
  </div>
...
{#if advanced}
  <form method="post" action="?/saveFull" class="mt-6" use:enhance>
    ...
    <!-- The mid-year ask. Someone who started trading in January and switched in
         July still files ONE return for the whole year, and Thalermark only knows
         about the part after they arrived. Nothing else in the product tells them
         this, and the tax worksheet can't tell on its own — so it gets said here,
         where they're already entering the past. -->
    <div class="callout mt-5 max-w-2xl">
      <p class="text-sm text-fg/70">
        <span class="text-fg">Already been trading this year?</span>
        Include what you've earned and spent since 1 January, not just what you own and owe. You file
        one tax return for the whole year, so anything you leave out here is missing from your tax
        worksheet later.
      </p>
    </div>

    <!-- Import. Fills the editor below; nothing is saved until the user submits,
         so what came out of their old software is reviewable first. -->
    <div class="mt-5 max-w-2xl rounded-sm border border-fg/10 bg-surface-2 p-4">
      ...
      <!-- Never silently dropped: an unmatched row is money the file had and the
           editor doesn't, which is exactly why it wouldn't balance. -->
