...
describe('welcome wizard steps', () => {
  it('numbers the last step last, not first', () => {
    ...
    expect(onboardingStepNumber('/welcome/books')).toBe(ONBOARDING_STEPS.length);
    expect(onboardingStepNumber('/welcome/books')).not.toBe(1);
  });

  it('numbers every step in route order', () => {
    expect(onboardingStepNumber('/welcome')).toBe(1);
    expect(onboardingStepNumber('/welcome/paid')).toBe(2);
    expect(onboardingStepNumber('/welcome/brand')).toBe(3);
    expect(onboardingStepNumber('/welcome/books')).toBe(4);
  });

  it('still falls back rather than crashing on an unknown path', () => {
    expect(onboardingStepNumber('/welcome/nope')).toBe(1);
  });
  ...
  it('treats both of the last step’s exits as finishing', () => {
    ...
    expect(ONBOARDING_FINISHED_FROM.has('/welcome/books')).toBe(true);
    expect(ONBOARDING_FINISHED_FROM.has('/welcome/brand')).toBe(true);
  });
  ...
  it('still counts leaving an early step as abandonment', () => {
    ...
    expect(ONBOARDING_FINISHED_FROM.has('/welcome')).toBe(false);
    expect(ONBOARDING_FINISHED_FROM.has('/welcome/paid')).toBe(false);
  });
  ...
  it('lists every route that actually exists under /welcome', () => {
    ...
    expect([...ONBOARDING_STEPS].sort()).toEqual(routes.sort());
  });
});
