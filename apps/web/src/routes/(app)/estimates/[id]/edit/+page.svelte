<script lang="ts">
  ...
  // value={form?.values?.X ?? data.estimate.X}; live-preview inputs (tax +
  // rows) use $state with untrack() initializers so SSR re-render after a
  // fail() captures the right values without firing the
  // state_referenced_locally warning.
  const values = $derived(form?.values);

  let tax = $state<string>(
    untrack(() => form?.values?.tax ?? (data.estimate.tax === '0.00' ? '' : data.estimate.tax)),
  );
...
<a href="/estimates/{data.estimate.id}" class="eyebrow text-ink/60 hover:text-ink">← Estimate {data.estimate.number}</a>
<h1 class="mt-3 font-serif text-4xl font-light leading-none tracking-tight text-ink">
  Edit estimate<span class="text-gold-deep">.</span>
</h1>
...
<form method="post" class="mt-8 space-y-8">
  <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
    <div>
      ...
      >
        ...
        {#each data.customers as c (c.id)}
          <option
            value={c.id}
            selected={(values?.customerId ?? data.estimate.customerId) === c.id}>{c.name}</option
          >
        {/each}
      </select>
      {#if err('customerId')}
    ...
    <div>
      <label for="expiresOn" class="font-mono text-xs uppercase tracking-widest text-ink/50">
        Expires
      </label>
      <input
        id="expiresOn"
      ...
      />
      {#if err('expiresOn')}
        <p class="mt-1 text-xs text-oxblood">{err('expiresOn')}</p>
      {/if}
  ...
  <div class="flex items-center gap-4">
    ...
    <a href="/estimates/{data.estimate.id}" class="text-sm text-ink/60 hover:text-ink">Cancel</a>
  </div>
</form>
