<script lang="ts">
  ...
  async function more() {
    ...
    loadError = false;
    try {
      const page = await fetchMore<Row>('/invoices/more', cursor, { status: filters.status });
      rows = [...rows, ...page.rows];
...
>
  ...
  {#if filters.status}
    <a href="/invoices" class="text-sm text-ink/60 hover:text-ink">Clear</a>
  {/if}
</form>

{#if rows.length === 0}
  <p class="mt-8 text-ink/70">
    {filters.status ? 'No invoices match this filter.' : 'No invoices yet.'}
  </p>
{:else}
